...
# ${BRIDGE_MARKER} — generated by \`jev bridge install\`. Do not edit by hand.
set -eu
...
function buildClaudeHooksPatch(scriptPath: string): HooksConfig {
  const handler = jevHookHandler(scriptPath, 'JEV Watchdog');
  return {
    PreToolUse: [{ hooks: [handler] }],
    PostToolUse: [
      {
        hooks: [{ ...handler, statusMessage: 'JEV Watchdog (post)' }],
      },
    ],
    UserPromptSubmit: [
      {
        hooks: [{ ...handler, statusMessage: 'JEV Watchdog (prompt)' }],
      },
    ],
  };
}
...
function installClaudeHooks(
  ...
  try {
    ...
    writeJsonFile(patchPath, {
      hooks: patch,
      note: 'Could not write .claude/settings.json (often blocked in sandboxed agents). Merge this hooks object into .claude/settings.json manually, or re-run `jev bridge install` in a normal terminal.',
      error: message,
    });
...
function installCodexHooks(hooksPath: string, scriptPath: string): void {
  ...
  writeJsonFile(hooksPath, {
    description: 'JEV Watchdog live bridge (merge-safe)',
    hooks: next,
  });
}
...
function uninstallCodexHooks(hooksPath: string): void {
  ...
  if (Object.keys(next).length === 0) {
    writeJsonFile(hooksPath, { description: 'JEV Watchdog live bridge (uninstalled)' });
  } else {
