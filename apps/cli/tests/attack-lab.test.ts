...
afterEach(() => {
  // runtime attack cleans its own workspace
  temps.length = 0;
});
...
describe('Stage 7 attack lab modes', () => {
  it('lists runtime-capable attacks separately', async () => {
    const { code, out } = await capture(['attack', '--list', '--runtime']);
    expect(code).toBe(0);
  ...
  it('runs prompt-injection-secret-access via real hooks (no simulateEvent)', async () => {
    ...
    const result = await runRuntimeAttackById('prompt-injection-secret-access');
  ...
  it('veyra attack --runtime exits 0 when contained', async () => {
    expect(existsSync(cli)).toBe(true);
    const { code, out } = await capture(['attack', '--runtime']);
    expect(code).toBe(0);
    expect(out).toContain('VEYRA ATTACK LAB');
    expect(out).toContain('RUNTIME');
    expect(out).toContain('1/1 controlled attack contained');
